...
class SDSTextrankMMRCustom(Model):
    def __init__(self, config: ModelConfig):
        super(SDSTextrankMMRCustom, self).__init__()
        self.config = config
        ...
        self.mmr_model = MMRSummarizer(config)

    def predict(self, cluster: Cluster) -> (List[str], List[float]):
        sents = cluster.get_all_sents()
        parser: PlaintextParser = PlaintextParser.from_string('  .  '.join(sents), self.tokenizer)
        ...
        for document in cluster.documents:
            sents = document.get_all_sents()
            chosen_sent, _ = self.get_summary(sents)
            finals += chosen_sent

        chosen_sent, ratings = self.mmr_model(finals, sent_count)

        return chosen_sent, ratings

    def get_summary(self, sents: List[str]) -> (List[str], List[float]):
        parser: PlaintextParser = PlaintextParser.from_string('  .  '.join(sents), self.tokenizer)
        ...
        assert len(parser.document.sentences) == len(sents), \
        ...
        return extracter(sents, list(ratings.values()), sent_count), list(ratings.values())


if __name__ == '__main__':
    from src.loader.class_loader import Cluster, load_cluster, Document
    ...
    textrank = SDSTextrankMMRCustom(config.models[0])
