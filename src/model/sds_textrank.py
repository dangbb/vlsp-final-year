...
class SDSTextrankCustom(Model):
    def __init__(self, config: ModelConfig):
        super(SDSTextrankCustom, self).__init__()
        self.config = config
    ...
    def predict(self, cluster: Cluster) -> (List[str], List[float]):
        ...
        chosen_sent, ratings = self.get_summary(finals)

        return extracter(chosen_sent, ratings, sent_count), ratings
...
if __name__ == '__main__':
    ...
    textrank = SDSTextrankCustom(config.models[0])
